...
.PHONY: build start test lint swag mock docker-build docker-run docker-push docker-run-postgres docker-run-mongodb docker-run-redis migrateup migratedown
.DEFAULT_GOAL := start
...
docker-run-mongodb:
	docker run --name ${MONGO_IMAGE} -p 27017:27017 -e MONGODB_DATABASE=${MONGODB_DATABASE} -d mongo:7
