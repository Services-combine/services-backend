		...
		auth := api.Group("/auth")
		{
			...
			user := auth.Group("/user")
			{
				...
				channels := user.Group("/channels")
				{
					...
					channels.POST("/add-mark", h.AddMark)
					channels.POST("/:channelID/update-mark", h.UpdateMark)
					channels.GET("/:channelID/delete-mark", h.DeleteMark)
					...
					channels.POST("/:channelID/edit-comment", h.EditComment)
					channels.POST("/:channelID/edit-proxy", h.EditProxy)
					channels.POST("/:channelID/edit-mark", h.EditMark)
				}
			}
		}
	}
