...
func (h *Handler) InitRoutes() *gin.Engine {
	...
	services := router.Group("/", h.userIdentity)
	{
		...
		inviting := services.Group("/inviting")
		{
			...
			inviting.GET("/:folderID/generate-interval")
			inviting.GET("/:folderID/check-block")
			inviting.GET("/:folderID/delete", h.DeleteFolder)
			inviting.GET("/:folderID/launch-inviting")
			inviting.GET("/:folderID/launch-mailing-usernames")
			inviting.GET("/:folderID/launch-mailing-groups")
